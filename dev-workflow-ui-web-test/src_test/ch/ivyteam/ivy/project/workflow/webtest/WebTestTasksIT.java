...
class WebTestTasksIT {
  ...
  void taskDetailsMessage() {
    ...
    loginDeveloper();
    Navigation.openTask("TaskDetailTest");
    infoMessage.shouldHave(text("You cannot work on the task because user 'testuser' has parked it."));
    warningMessage.shouldBe(hidden);
    ...
    Navigation.openTask("TaskDetailTest");
    infoMessage.shouldHave(text("Task has already been completed by user 'testuser'."));
    warningMessage.shouldBe(hidden);
  }
